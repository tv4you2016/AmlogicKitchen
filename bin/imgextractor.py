...
class Extractor(object):
    ...
    def __ext4extractor(self):
        ...
        contexts = self.CONFING_DIR + os.sep + \
            self.FileName + "_file_contexts"  # 08.05.18
       ...
       # f.close()
        with open(self.OUTPUT_IMAGE_FILE, 'rb') as file:
            ...
            dirr = self.__file_name(os.path.basename(
                self.OUTPUT_IMAGE_FILE).split('.')[0])  # 11.05.18
    ...
    def main(self, target, output_dir):
        ...
        self.EXTRACT_DIR = os.path.realpath(os.path.dirname(
            output_dir)) + os.sep + self.__file_name(os.path.basename(output_dir))  # output_dir
        ...
        if target_type == 'simg':
            print(".....Convert %s to %s" % (os.path.basename(target),
            ...
            print(".....Extraction from %s to %s" %
                  (os.path.basename(target), os.path.basename(self.EXTRACT_DIR)))
            self.__ext4extractor()
            print(".....Done! All extraction in %s" %
                  (os.path.basename(self.EXTRACT_DIR)))
        if target_type == 'img':
            with open(os.path.abspath(self.OUTPUT_IMAGE_FILE), 'rb') as f:
                data = f.read(500000)
            moto = re.search(b'\x4d\x4f\x54\x4f', data)
    ...
    if sys.argv.__len__() == 3:
        ...
        if sys.argv.__len__() == 2:
            if not os.path.isdir("out"):
                os.makedirs("out")
            Extractor().main(sys.argv[1], "out" + os.sep +
